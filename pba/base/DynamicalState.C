  ...
  name (nam)
{
   ...
   vector_attributes["pos"] = DSAttribute<Vector>( "pos", Vector(0,0,0) );
    vector_attributes["fixpos"] = DSAttribute<Vector>( "fixpos", Vector(0,0,0) );
   vector_attributes["vel"] = DSAttribute<Vector>( "vel", Vector(0,0,0) );
    vector_attributes["force"] = DSAttribute<Vector>( "force", Vector(0,0,0) );
...
const Vector& DynamicalStateData::fixpos( const size_t p ) const
{
    return fixedpositions->second.get(p);
}
const Vector& DynamicalStateData::force( const size_t p ) const
{
    return forces->second.get(p);
}
...
void DynamicalStateData::set_fixpos( const size_t p, const Vector& value )
{
    fixedpositions->second.set(p, value);
}
void DynamicalStateData::set_force( const size_t p, const Vector& value )
{
    forces->second.set(p, value);
}
...
void DynamicalStateData::re_find_main_attrs()
{
   ...
   if( positions == vector_attributes.end() )
   {
    ...
    fixedpositions = vector_attributes.find( "fixpos" );
    if( fixedpositions == vector_attributes.end() )
    {
        std::cout << "ERROR could not find fixedpositions\n";
    }
   ...
   if( velocities == vector_attributes.end() )
   {
    ...
    forces = vector_attributes.find( "force" );
    if( forces == vector_attributes.end() )
    {
        std::cout << "ERROR could not find forces\n";
    }
